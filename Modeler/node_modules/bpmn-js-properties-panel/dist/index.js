  ...
  return props => {
    ...
    hooks.useEffect(() => {
      const extractVariables = async () => {
        const variables = await zeebe.getVariablesForElement(bo);
...
const FeelEntryWithContext = withVariableContext(withTooltipContainer(propertiesPanel.FeelEntry));
const FeelTextAreaEntryWithContext = withVariableContext(withTooltipContainer(propertiesPanel.FeelTextAreaEntry));
...
function areInputParametersSupported$1(element) {
  return ModelingUtil.isAny(element, ['bpmn:UserTask', 'bpmn:SubProcess', 'bpmn:CallActivity', 'bpmn:BusinessRuleTask', 'bpmn:ScriptTask']) || isZeebeServiceTask(element);
}
 ...
 */
class UpdateTemplatePropertiesOrder extends CommandInterceptor__default["default"] {
  constructor(eventBus, elementTemplates, commandStack, bpmnFactory) {
    super(eventBus);
    this._eventBus = eventBus;
    this._elementTemplates = elementTemplates;
