...
async function getConfiguration() {
  ...
  const configFileUrl = '../../../../../..'+path+'/descriptors/tenantConfig.json'; // Inserisci il percorso del file di configurazione JSON
  const response = await fetch(configFileUrl);
  const configData = await response.json();
  const currentDomain = window.location.hostname + ':' + window.location.port;

  const configEntry = configData.urls.find(entry => entry.domain === currentDomain);

  if (!configEntry) {
    throw new Error(`Configuration not found for domain: ${currentDomain}`);
  }
...
async function generateFromAPI() {
  ...
  config.popupmenu_url = path+config.popupmenu_url;
  const response = await fetch(config.popupmenu_url);
  const jsonData = await response.json();

  const customGroups = {};
  const customItems = [];

  jsonData.forEach(({ id, name, items }) => {
    if (!customGroups[id]) {
      customGroups[id] = {
        id,
        name
...
// async function init() {
//   //const { groups, items } = await generateFromAPI();
