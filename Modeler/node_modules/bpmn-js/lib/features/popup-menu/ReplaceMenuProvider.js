...
import { dynamicVariablesPromise } from '../replace/ReplaceOptions';
...
//aggiungo le variabili esportare personalizzate in base alla chiamata API
async function handleEntries() {
  var othervariables = await dynamicVariablesPromise;
  return othervariables;
}

(async function main() {
  var altre_variabili = await handleEntries();
  //console.log(altre_variabili);

  // Aggiungere le proprietà di altre_variabili a replaceOptions
  Object.assign(replaceOptions, altre_variabili);
...
async function loadConfig() {
  ...
  const response = await fetch('../../../../..'+path+'/descriptors/tenantConfig.json');
  const config = await response.json();
  return config;
}

//recupero i tipi ed i loro gruppi associati
async function fetchAndProcessData() {
  try {
    const config = await loadConfig();
    const domain = window.location.hostname + ':' + window.location.port;
    let path = window.location.pathname;
    let typesUrl = ''; // URL di default

    for (const urlInfo of config.urls) {
      if (urlInfo.domain === domain) {
        typesUrl = urlInfo.types_url;
        break;
      }
    }

    typesUrl = path+typesUrl;
...
var group_types = await fetchAndProcessData();
//console.log(gropu_types);
...
ReplaceMenuProvider.prototype.getEntries = function(element) {
  ...
  // flow nodes e gestione replace menu con elementi dello stesso gruppo personalizzato
  if (is(businessObject, 'bpmn:FlowNode')) {
    const foundElement = group_types.find((item) => item.name === element.type);
    if (foundElement) {
      const group = foundElement.group;
      var filtro = replaceOptions[group];
      entries = filter(filtro, differentType);
    }
    // if(element.type === 'elmi:CustomTask' || element.type === 'elmi:SpecialTask'){
    //   entries = filter(replaceOptions.CUSTOM_TASK, differentType);
    // }
    else{
