...
//chiama api per il get del dmn
app.get(basePath+'/api/getDmn/:id', keycloak.protect(), async function(req, res) {
...
app.post(basePath+'/api/postDmn/:id', keycloak.protect(), async function(req, res) {
  var token = req.kauth && req.kauth.grant && req.kauth.grant.access_token.token;
  var id = req.params.id; // Prendi l'ID dal parametro della rotta
...
htmlData = htmlData.replace(new RegExp(`href="(?!${basePath})(/[^"]*\\.css[^"]*)"`, 'g'), `href="${basePath}$1"`);
htmlData = htmlData.replace(new RegExp(`src="(?!${basePath})(/[^"]*\\.js[^"]*)"`, 'g'), `src="${basePath}$1"`);
