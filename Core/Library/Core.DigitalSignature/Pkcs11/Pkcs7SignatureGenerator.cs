...
namespace Core.DigitalSignature.Pkcs11
{
    ...
    public class Pkcs7SignatureGenerator : IDisposable
    {
        ...
        public Pkcs7SignatureGenerator(string libraryPath, string tokenSerial, string pin, string ckaId, HashAlgorithm hashAlgorihtm, SignatureScheme signatureScheme)
        {
            byte[] pinValue = (pin == null) ? null : ConvertUtils.Utf8StringToBytes(pin);
            byte[] ckaIdValue = (ckaId == null) ? null : ConvertUtils.Utf8StringToBytes(ckaId);
            InitializePkcs7RsaSignature(libraryPath, tokenSerial, "", pinValue, "", ckaIdValue, hashAlgorihtm, signatureScheme);
        }
        ...
        public Pkcs7SignatureGenerator(string libraryPath, ulong slot, string pin, string ckaLabel, string ckaId, HashAlgorithm hashAlgorihtm, SignatureScheme signatureScheme)
        {
            try
            {
                byte[] ckaIdValue = (ckaId == null) ? null : ConvertUtils.HexStringToBytes(ckaId);
                InitializePkcs7RsaSignature2(libraryPath, slot, pin, ckaLabel, ckaIdValue, hashAlgorihtm, signatureScheme);
            } catch (Exception ex)
            {
        ...
        private void InitializePkcs7RsaSignature(string libraryPath, string tokenSerial, string tokenLabel,  byte[] pin, string ckaLabel, byte[] ckaId, HashAlgorithm hashAlgorihtm, SignatureScheme signatureScheme)
        {
            try
            {
                ...
                _pkcs11 = Settings.Factories.Pkcs11LibraryFactory.LoadPkcs11Library(Settings.Factories, libraryPath, AppType.MultiThreaded);
        ...
        private void InitializePkcs7RsaSignature2(string libraryPath, ulong slot, string pin, string ckaLabel, byte[] ckaId, HashAlgorithm hashAlgorihtm, SignatureScheme signatureScheme)
        {
            try
            {
                if (string.IsNullOrEmpty(libraryPath))
                ...
                List<ISlot> slots = _pkcs11.GetSlotList(SlotsType.WithTokenPresent);
                _slot = slots.FirstOrDefault(s => s.SlotId == slot);
                if (_slot == null)
                    throw new TokenNotFoundException(string.Format("Token with id \"{0}\" was not found", slot));
        ...
        public byte[] GenerateSignature(byte[] data, bool detached, BCX509.X509Certificate signingCertificate, ICollection<BCX509.X509Certificate> certPath)
        {
            ...
            //var ts = tsaClient.GetTimeStampToken(dataHash);
            ...
            //        attrValues: new DerSet(new DerOctetString(t.Digest()))));
            ...
            if (_signatureScheme == SignatureScheme.RSASSA_PKCS1_v1_5)
            {
                ...
                using (Net.Pkcs11Interop.HighLevelAPI.ISession session = _slot.OpenSession(SessionType.ReadOnly))
                using (Net.Pkcs11Interop.HighLevelAPI.IMechanism mechanism = Settings.Factories.MechanismFactory.Create(CKM.CKM_RSA_PKCS))
            ...
            else if(_signatureScheme == SignatureScheme.RSASSA_PSS)
            {
                ...
                ICkRsaPkcsPssParams pssMechanismParams = CreateCkRsaPkcsPssParams(_hashAlgorihtm);
        ...
        private IObjectHandle FindPrivateKey(string ckaLabel, byte[] ckaId)
        {
            ...
            {
                List<IObjectAttribute> searchTemplate = new List<IObjectAttribute>();
                searchTemplate.Add(_session.Factories.ObjectAttributeFactory.Create(CKA.CKA_CLASS, CKO.CKO_PRIVATE_KEY));
                searchTemplate.Add(_session.Factories.ObjectAttributeFactory.Create(CKA.CKA_KEY_TYPE, CKK.CKK_RSA));
                if (!string.IsNullOrEmpty(ckaLabel))
                    searchTemplate.Add(_session.Factories.ObjectAttributeFactory.Create(CKA.CKA_LABEL, ckaLabel));
                if (ckaId != null)
                    searchTemplate.Add(_session.Factories.ObjectAttributeFactory.Create(CKA.CKA_ID, ckaId));

                List<IObjectHandle> foundObjects = _session.FindAllObjects(searchTemplate);
                if (foundObjects.Count < 1)
