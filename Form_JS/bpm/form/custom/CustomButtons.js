...
async function getFormPreviewInBase64() {
  const formElement = document.getElementsByClassName("fjs-children fjs-drag-container fjs-vertical-layout"); // ID del tuo form
  const canvas = await html2canvas(formElement[0]);
...
export class SaveDiagramButton {
  constructor(formModeler, containerEl) {
    this._formModeler = formModeler;
  ...
  _createLoader() {
    ...
    text.style.top = '60%';  // Posiziona il testo un po' sotto al spinner
    text.style.left = '51%';
  ...
  async _confirmSave() {
    const { isConfirmed } = await Swal.fire({
      title: 'Sei sicuro?',
      text: "Sei sicuro di voler salvare il form?",
      icon: 'warning',
  ...
  async _saveDiagram() {
    ...
    try {
      // Estrai il form come JSON
      const schema = await this._formModeler.getSchema();
    
      // Converti l'oggetto schema in una stringa JSON
      ...
      if (response == 200) {
        ...
        Swal.fire({
          title: 'Salvato!',
          text: "Il tuo form è stato salvato correttamente.",
          icon: 'success',
...
export class PublishDiagramButton {
  ...
  _createButton() {
    ...
    button.textContent = "Pubblica form";
    button.className = 'panel';
    button.style.marginRight = '5px';
    button.style.zIndex = '10';
    button.style.cursor = 'pointer';
  ...
  async _confirmSave() {
    const { isConfirmed } = await Swal.fire({
      title: 'Sei sicuro?',
      text: "Sei sicuro di voler pubblicare il form?",
      icon: 'warning',
      showCancelButton: true,
  ...
  async _saveDiagram() {
    ...
    try {
      ...
      if (response == 200) {
        ...
        Swal.fire({
          title: 'Pubblicato!',
          text: "Il tuo form è stato pubblicato correttamente.",
          icon: 'success',
          confirmButtonColor: 'blue',
        });
      } else {
...
async function postDiagram(content, previewInBase64, toBePublished) {
  ...
  if (firstEntry) {
    ...
    const response = await fetch(mypathbase+`api/postForm/${id}`, {
      method: 'POST',
      headers: {
