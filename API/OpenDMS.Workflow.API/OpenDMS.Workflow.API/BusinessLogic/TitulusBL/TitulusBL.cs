...
using File = OpenDMS.Workflow.API.DTOs.Titulus.File;
    ...
    public class TitulusBL : ITitulusBL
    {
        ...
        public string GetDocumentFromProtocol(string numero_protocollo)
        {
            try
            {
                ...
                if (docElement.ValueKind == JsonValueKind.Null)
                {
                    return "";                
                }
                else
        ...
        public DocSearch GetFiles(string jsonString)
        {
            ...
            foreach (FileSearch file in filesList)
            {
                ...
                file.Principale = filesListLoaded.Find(item => item.Name.ToLower() == file.Name.ToLower()).Principale;
