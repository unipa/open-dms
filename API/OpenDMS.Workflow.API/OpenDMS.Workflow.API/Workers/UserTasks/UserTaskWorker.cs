...
{
    public class UserTask : BaseWorker
    {
        private readonly IUserTaskService taskService;

        public override string JobType { get; set; } = "userTask";
        public override string TaskLabel { get; set; } = "Attività Utente";
        public override string Icon { get; set; } = "fa fa-users";
        public override string GroupName { get; set; } = "Task Utente";
        public override string[] AlternativeTasks { get; set; } = new[] { "requestForEventTask", "userMessageTask", "completeTask" };
        public override string Inputs { get; set; } = "Title,Description,CompanyId,ExpirationDate,CategoryId,ProjectId,ParentTaskId,AssignToAllUsers,NotifyTo,NotifyCC,NotifyExecution,NotifyExpiration,PriorityId,Sender,Attachments";
        ...
        public override async Task<string> Execute (IJob job)
        {
            ...
            if (TaskId <= 0)
            {
                ...
                if (String.IsNullOrEmpty(TaskInputVariable.Title))
                    TaskInputVariable.Title = "Richiesta di Attività";
                ...
                TaskInputVariable.TaskType = TaskInputVariable.NotifyTo.Count > 0 ? Domain.Enumerators.TaskType.Activity : Domain.Enumerators.TaskType.Message;
