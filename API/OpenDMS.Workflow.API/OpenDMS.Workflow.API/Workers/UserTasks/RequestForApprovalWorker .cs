...
{
    public class RequestForApprovalWorker : BaseWorker
    {
        private readonly IUserTaskService taskService;

        public override string JobType { get; set; } = "requestForApprovalTask";
        public override string TaskLabel { get; set; } = "Richiesta Approvazione";
        public override string Icon { get; set; } = "fa fa-thumbs-up";
        ...
        public override string Inputs { get; set; } = "Title,Description,CompanyId,ExpirationDate,CategoryId,ProjectId,ParentTaskId,AssignToAllUsers,FormKey,NotifyTo,NotifyExecution,NotifyExpiration,PriorityId,Sender,Attachments,Links";
        public override string Outputs { get; set; } = "Choise,TaskId,TaskUser";



        public RequestForApprovalWorker(
            ILogger<RequestForApprovalWorker> logger,
        ...
        public override async Task<string> Execute(IJob job)
        {
            ...
            if (TaskId <= 0)
            {
                ...
                if (String.IsNullOrEmpty(TaskInputVariable.Title))
                    TaskInputVariable.Title = "Richiesta di Approvazione";
                ...
                //string[] Choises = ((string?)JObject.Parse(job.Variables)["Choises"] ?? "").Split(",", StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
                //TaskInputVariable.FormSchema = FormSchema.AskForApproval(TaskInputVariable.Description, Choises, true);
