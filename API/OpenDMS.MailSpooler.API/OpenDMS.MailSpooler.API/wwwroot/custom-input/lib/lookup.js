...
export default class Lookup extends HTMLElement {
  ...
  createNestedList = (obj) => {
    ...
    return `<li><div class="parent-item"><div id="${
      ...
      obj.codice
    }</span></div><div class="toggle-button">${chevronIcon}</div></div><ul class="child-list">
      ${arrayPresent.map((item) => this.createNestedList(item)).join("")}
    </ul></li>`;
  };
  /* ---- If there are no arrays in obj ---- */
  createSingleValue = (obj) => {
    return `<li><div id="${obj.codice}" class="selectable-item selectable-without-chevron"><span>${obj.name}</span><span>${obj.codice}</span></div></li>`;
  };
  /* ----------------------- */
