...
{
    public class SenderHostingService : IHostedService, IDisposable
    {
        //        private IMailSenderService _mailSpooler = null;
        //        private IMailEntryService _mailEntryRepository = null;
        private readonly IServiceProvider _serviceProvider;
        private readonly ILogger<SenderHostingService> _logger;
        ...
        public SenderHostingService(IServiceProvider serviceProvider, ILogger<SenderHostingService> logger, IConfiguration configuration)
        {
        ...
        private void StartThreads()
        {
            ...
            lock (LockObject)
            {
                ...
                _logger.LogInformation("Mail Sender: Starting " + (threads - _timers.Count()) + " new threads");
            ...
            try
            {
                ...
                using (var scopedService = _serviceProvider.CreateScope())
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        _logger.LogError(ex, "Mail Sender (DoWork) - EXCEPTION:");
                    }
                }
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Mail Sender (DoWork:Services) - EXCEPTION:");
                //Loggare l'errore
            }
            StartThreads();
