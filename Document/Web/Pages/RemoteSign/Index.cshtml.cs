...
namespace Web.Pages.RemoteSign
{
    ...
    public class IndexModel : PageModel
    {
        ...
        public async Task OnGetAsync(string documents, SignTypes type = SignTypes.Pades, string signField = "", bool fromActivity = false)
        {
            ...
            try
            {
                ...
                {
                    await OnPostConfirm(this.Documents, SignTypes.Pades, signField);
                }
                else
                {
                ...
                {
                    logger.LogWarning("RemoteSign/Index:" + ErrorMessage);
                }
            }
            catch (Exception ex)
            {
                logger.LogError(ex, "RemoteSign/Index");
        ...
        private async Task<string> CheckSignField(bool checkEmpty = true)
        {
            ...
            {
                string sfield = signField;
                if (!SignFields.Any(f => f.Value.Any(f => f.Name == sfield)))
                {
                    signField = "";
        ...
        public async Task<IActionResult> OnPostConfirm(string documents, SignTypes type = SignTypes.Pades, string signField = "", bool fromActivity = false)
        {
            ...
            try
            {
                ...
                if (!String.IsNullOrEmpty(ErrorMessage))
                {
                    logger.LogWarning("RemoteSign/Index/Post:" + ErrorMessage);
                }
            }
            catch (Exception ex)
            {
                logger.LogError(ex, "RemoteSign/Index/Post");
                ErrorMessage = ex.Message;
            }
            return Page();
