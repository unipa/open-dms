    ...
    $.widget("elmi.List", $.extend({}, { // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!   list.js modificato per boostrap italia 
        ...
        _init: function () {
            ...
            if (this.options.textField) {
                var $ul = $("<ul class='nav-list pt-4 " + (this.options.commentField ? "Lista3" : "Lista1")+"'></ul>").addClass(this.options.className).attr("data-bind", "foreach: " + this.options.items + "").appendTo(this.element);
                var $li = $("<li class=''></li>").appendTo($ul);
                var $item = $li;
                var $a = $(`<a data-bind='visible:  ` + that.options.idField + `() !=\"-\",  click: $root.` + c + `, attr: { itemid: ` + (this.options.idField) + `}'` + (this.options.commentField ? "" : '')+`>
                            </a>`).appendTo($item);
                ...
                $a.append("<span class='me-0' data-bind='text: " + this.options.textField + "'></span>");
