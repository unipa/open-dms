...
{
    [Authorize]
    [Route("internalapi/identity/[controller]")]
    [ApiController]
    public class AvatarController : ControllerBase
    {
        ...
        [HttpGet("default/{subjectId}")]
        public async Task<FileResult> GetDefaultImage(string subjectId)
        {
            ProfileType profileType = (ProfileType)int.Parse(subjectId.Substring(0, 1));
            string profileId = subjectId.Substring(1);
            var data = await userContext.GetDefaultAvatar(profileId, profileType);
            return File(data, System.Net.Mime.MediaTypeNames.Image.Jpeg, subjectId + ".png");
        }

        [HttpGet("{subjectId}/{groupid}")]

        public async Task<FileResult> GetImage(string subjectId, string groupid = "")
        {
