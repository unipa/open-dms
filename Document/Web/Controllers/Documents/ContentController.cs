...
public class ContentController : ControllerBase
{
    ...
    /// <summary>
    /// Metodo usato per caricare l'immagine a un documento dalla pagina di Drag&Drop
    /// </summary>
    [RequestSizeLimit(4_000_000_000)]
    [HttpPost("uploadcontent/{DocId}")]
    public async Task<ActionResult> UploadContent(int DocId, IFormFile file)
    {
        try
        {
            if (DocId <= 0) throw new Exception("Documento non trovato");

            if (!ModelState.IsValid)
            {
                throw new Exception("I dati non sono stati caricati correttamente.");
            }
            ...
            var i = await docRepo.AddContent(DocId, u, fileContent, true);
            if (i == null)
                return BadRequest("Errore durante il salvataggio. Verificare i dati inseriti");
            return Ok();
        }
        catch (Exception ex)
        {
            ...
            //throw ex;// new Exception("Errore durante il salvataggio. Errore: " + ex.Message);
        }
    ...
    ///// <summary>
    ///// Ritorna il contenuto del documento in streaming
    ///// </summary>
    ///// <param name="documentId">Identificativo del documento</param>
    ///// <returns></returns>
    //[HttpGet("streaming/{documentId}")]
    //public async Task<ActionResult> Streaming(int documentId)
    //{
    //    var u = userContext.Get();
    //    var d = await docRepo.Load(documentId, u);
    //    if (d == null || d.Image == null) return NotFound(); ;
    ...
    //    var content = await docRepo.GetContent(d.Image.Id);
    //    return new FileStreamResult(content, "application/octet-stream") { FileDownloadName = Path.Combine(d.Image.OriginalFileName) };
    //    return d;
    //}
